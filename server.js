...
// OBTENER productos (Filtrado por usuario y opcionalmente categoría)
app.get("/stock", (req, res) => {
  const { categoria } = req.query;
  const usuario = req.headers["x-usuario"]; // Leer cabecera de usuario
  ...
  db.query("SELECT id FROM usuarios WHERE usuario = ?", [usuario], (err, users) => {
    ...
    // 2. Traer productos que pertenezcan EXCLUSIVAMENTE a este usuario
